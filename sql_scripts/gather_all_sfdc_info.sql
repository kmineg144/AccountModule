...
drop table BI_MIP.dbo.ModuleAccts
...
select * from BI_MIP.dbo.ModuleAccts where SavilleBillingAccountNumber__c = '939023198'


-------------------------------scratch work--------------------------------------

select * from #yao1 where SV_ROOT_ACCOUNT = '900007769'
select * from RPTCB.RPT.ModuleAccts  where Billing_Account__c = '900007769'
select count(*) from BI_MIP.dbo.ModuleAccts
...
create nonclustered index ix_tempModuleAccts1 on BI_MIP.dbo.ModuleAccts (ParentId)
create nonclustered index ix_tempModuleAccts2 on BI_MIP.dbo.ModuleAccts (id)
create nonclustered index ix_tempModuleAccts3 on BI_MIP.dbo.ModuleAccts(Site__c)
create nonclustered index ix_tempModuleAccts4 on BI_MIP.dbo.ModuleAccts (Location_Id__c)

drop table BI_MIP.dbo.gotem_dyao
...
FROM (
		...
		join BI_MIP.dbo.ModuleAccts as p
...
select * from BI_MIP.dbo.gotem_dyao  where Billing_Account__c = '900007769'
select count(*) from BI_MIP.dbo.ModuleAccts
select count(*) from rptcb.rpt.gotem_dyao

--select * from rptcb.rpt.gotem_dyao where ACCOUNT__C ='0011H00001ZL09yQAD'
----------------------------------------------------------------------------------
...
drop table BI_MIP.dbo.needem_dyao  

select 
	* 
into 
	BI_MIP.dbo.needem_dyao  
from 
	BI_MIP.dbo.ModuleAccts
where 
	id not in ( select distinct ACCOUNT__C FROM BI_MIP.dbo.gotem_dyao )
...
FROM (
	...
	JOIN BI_MIP.dbo.needem_dyao  AS M ON M.id = L.Account__c AND  l.account__C is not null 
...
,StillNeedEm as(
select * from BI_MIP.dbo.ModuleAccts 
...
into BI_MIP.dbo.sfdc_to_biller_dyao
from
(select 
	*
...
--new: 5144337
--old: 5100049
select count(*) from BI_MIP.dbo.sfdc_to_biller_dyao where csg3 <> ''--where SV2 = '900007769'
